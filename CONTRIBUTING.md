...
Human-authored changes use ordinary feature branches and pull requests.
Run the repository's focused tests and lint checks before pushing, then let GitHub Actions run the complete required suite.

## Workflow

1. Fork the repo and configure your fork as `origin` and the parent repository as `upstream`.
2. Create a branch and make your changes.
3. Run focused tests and `bin/fm-lint.sh`.
...
6. Open a pull request against the intended base repository and wait for required CI checks.
...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/` - ship through a direct pull request on a feature branch and require an explicit merge approval.
