...
# re-ring ladder. The composer pre-check before the ring is ADVISORY: a
# composer holding other visible pending text is skipped with a notice, while
# the exact Firstmate doorbell is submitted in place without being retyped.
# The backend must verify a fresh doorbell's submission, retrying Enter without
# retyping; a failed or unconfirmed ring still leaves the durable record sent
# and the watcher owns later retries.
#
