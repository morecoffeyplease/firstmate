...
Ship shared tracked changes through the direct PR path, with the same merge authority as any other project.
...
Follow the selected delivery path without adding an independent review pipeline.
A separate review or audit is allowed only when the captain explicitly requests that deliverable or the authorized task is a knowledge-only review; one named question remains scoped to that question.
If a task needs more rigor, use the repository's tests, lint, and CI and report any remaining risk explicitly.
The path's worker, automated gates, and captain approval remain authoritative:

- **direct-PR** has the worker run focused repository validation, push, and open a PR, then waits for the configured merge authority.
...
Require the matching `resolved` event and require the worker to process every synchronous return until completion or a genuinely new escalation.
Resume fleet supervision immediately after the decision lands.

The worker runs the repository's focused tests and lint checks before pushing, reports failures plainly, and reports the PR after it opens.
The forge remains authoritative for required CI checks before merge.

### PR ready, landing, and teardown

For PR-based ship tasks, `direct-PR` reports `done: PR <url>` after opening the PR.
...
`bin/fm-dod-lib.sh` owns intent authoring and its rule that the string must be self-sufficient.
