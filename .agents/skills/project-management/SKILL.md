...
- `direct-PR` pushes and opens a PR after the repository's focused validation.
...
Legacy registry entries are interpreted by `bin/fm-project-mode.sh`, which defaults entries without an annotation to `direct-PR`.
